...
export function PrescriptionStatusDashboard() {
  const navigate = useNavigate();
  const [sessions, setSessions] = useState<Session[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getSessions('prescription_status')
      .then(setSessions)
      .catch(console.error)
  ...
  if (loading) {
    return (
      <WorkflowLayout workflowName="prescription_status" title="Dashboard">
  ...
  return (
    <WorkflowLayout
      ...
      title="Dashboard"
      actions={
        <Button onClick={() => navigate('/workflows/prescription_status/calls')}>
          View All Calls
        </Button>
      }
    >
